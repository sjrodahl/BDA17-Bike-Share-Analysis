...
ByAgeGroup<- group_by(trips_df, sHour, Agecat1)

SumByAgeGroup <- dplyr::summarize(ByAgeGroup,rentals = n())
#bug fixed: grouping by agecat1 and then sHour doesn't produce correct plot
hourlyByAge<-ggplot(ByAgeGroup, aes(x=as.factor(sHour), y = rentals, colour = Agecat1, group = Agecat1))+
     geom_point(data = SumByAgeGroup, aes(group = Agecat1)) +
     geom_line(data = SumByAgeGroup, aes(group = Agecat1)) +
     scale_x_discrete() +
     #scale_y_continuous(breaks = seq(0,900,100))+
     xlab('Hour')+
     ylab('rentals')
...
#plot over timediff vs usertypes and times
TimeVsAge <- group_by(trips_df,sHour,Agecat1,timeDiff) #%>% filter(!is.na(usertype))
SumByTimeAge<-dplyr::summarize(TimeVsAge,median = median(timeDiff))
...
UserTypePlot<-ggplot(ByUserType, aes(x = as.factor(sHour), y = rentals, colour = usertype))+
  ...
  ylab('rentals')+
  theme_minimal() 
#plot over timediff vs agegroups
#plot over timediff vs weather
#
